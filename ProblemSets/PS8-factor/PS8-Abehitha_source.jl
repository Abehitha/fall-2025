# You may need to include an external quadrature file
include("lgwt.jl")
...
# Hints:
- asvabs = Matrix(df[:, end-5:end])'  (note the transpose!)
...
1. Extract ASVAB scores as a matrix
2. IMPORTANT: Transpose to J×N (MultivariateStats requires features × observations)
3. Fit Factor Analysis model with maxoutdim=1
4. Transform the data to get factor scores
5. Add factor scores to DataFrame and run regression

# Hints:
- asvabs = Matrix(df[:, end-5:end])'  (note the transpose!)
- M = fit(FactorAnalysis, asvabs; maxoutdim=1)
- asvabFactor = MultivariateStats.transform(M, asvabs)
- asvabFactor will be 1×N, need to reshape for regression
"""
function generate_factor!(df::DataFrame)
    ...
    asvabFactor = MultivariateStats.transform(M, asvabs)
    # Add factors to dataframe (careful with dimensions!)
...
function factor_model(θ::Vector{T}, X::Matrix, Xfac::Matrix, Meas::Matrix, 
    ...
    for r in 1:R
        ...
        for j in 1:J
            ...
            #Mlike[:, j] = pdf.(Normal(Mres, sdj)) #This should be equivalent
        end
        ...
        #Ylike = pdf.(Normal(Yres, sdy)) #This should be equivalent
...
function run_estimation(df::DataFrame, start_vals::Vector)
    ...
    #return estimates, standard errors, and log-likelihood value
...
function main()
    ...
    println("\nMulticollinearity is a problem; it doesn't make sense to 'hold all else equal' if the focal variable is highly correlated with other variables in the model. They may all be derived from some latent common factor which is what we should be exogeneously varying.")
